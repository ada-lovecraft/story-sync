            ...
            try {
                ...
                if (bestChunks.length > 1) {
                    ...
                    roundChunks = bestChunks.map((chunk, index) => {
                        ...
                        if (roundMatch && (roundMatch[1] || roundMatch[2])) {
                            // This assertion lets the compiler know we're handling the undefined case
                            const capturedValue = (roundMatch[1] || roundMatch[2]) as string;
                            const parsed = parseInt(capturedValue, 10);
                            if (!isNaN(parsed)) {
                                roundNumber = parsed;
                            }
                        }
