...
export function useWorkflowData() {
    ...
    const goToNextStep = () => {
        ...
        if (nextStepValue <= WorkflowStep.CHAPTER) {
            ...
            const route = WORKFLOW_ROUTES[nextStep];
            if (route) {
                log("Navigating to next step", { route, step: nextStep });
                router.push(route);
            }
        }
    };
    ...
    const goToPreviousStep = () => {
        ...
        if (prevStepValue >= WorkflowStep.UPLOAD) {
            ...
            const route = WORKFLOW_ROUTES[prevStep];
            if (route) {
                log("Navigating to previous step", { route, step: prevStep });
                router.push(route);
            }
        }
    };
