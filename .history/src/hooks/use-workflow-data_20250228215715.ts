...
// Create store with zustand and persist middleware
        ...
        (set) => ({
            ...
            navigateToStep: (step: WorkflowStep) => {
                ...
                if (route && typeof window !== "undefined") {
                    // Use window.location.href for navigation - this will cause a page reload
                    // but the data will be persisted in localStorage
                    window.location.href = route;
                }
            }
        }),
        {
            name: "workflow-storage", // Name for the localStorage key
        }
    )
);
...
export function useWorkflowData() {
    ...
    const goToNextStep = () => {
        ...
        if (Number(nextStepValue) <= Number(WorkflowStep.CHAPTER)) {
    ...
    const goToPreviousStep = () => {
        ...
        // Convert to number for safe comparison
        if (Number(prevStepValue) >= Number(WorkflowStep.UPLOAD)) {
