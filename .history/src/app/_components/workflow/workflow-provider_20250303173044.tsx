...
export function WorkflowProvider({
    ...
    const goToStep = useCallback(
        (step: WorkflowStep) => {
            setCurrentStep(step);
            // Add the activeFileId as a query parameter if it exists
            const route = activeFileId
                ? `${WORKFLOW_STEPS[step].route}?chatLogId=${activeFileId}`
                : WORKFLOW_STEPS[step].route;
            router.push(route);
        },
        [router, activeFileId]
    );
