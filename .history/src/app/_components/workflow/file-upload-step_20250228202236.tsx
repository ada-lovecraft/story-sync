...
export function FileUploadStep() {
    ...
    const { data: chatLogsData, isLoading: isLoadingDocuments, error: chatLogsError } = api.workflow.getAllChatLogs.useQuery(undefined, {
        enabled: true
    });
        ...
        if (chatLogsData && Array.isArray(chatLogsData)) {
            ...
            try {
                const mappedDocuments: Document[] = chatLogsData.map((logItem: ChatLog) => ({
                    ...
                    size: logItem.size,
                    date: new Date(logItem.updatedAt),
                    uploadDate: new Date(logItem.updatedAt),
                ...
                err("Error mapping documents", { error: String(error) });
    ...
    useEffect(() => {
        if (chatLogsData === undefined && !isLoadingDocuments && !chatLogsError) {
            ...
            err("TRPC error loading documents", { error: String(chatLogsError) });
            toast.error("Failed to fetch documents. Please check your connection and try again.");
        }
    }, [chatLogsData, isLoadingDocuments, chatLogsError]);
