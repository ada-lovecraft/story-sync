...
export function RoundsStep({ className }: RoundsStepProps) {
    ...
    // Get workflow data
    const { workflowData, updateWorkflowData, setNextEnabled, navigateToStep } = useWorkflowData();
    ...
    // Get the fileId from the workflow context
    const fileId = selectedDocumentId;

    // Get the selected document name from chat logs if available
    const selectedDocument = workflowData.chatLogs?.find((log: ChatLog) => log.id === fileId);
    const documentName = selectedDocument?.filename ?? "Selected document";
    ...
    const handleContinueToChapters = useCallback(() => {
        ...
        navigateToStep("CHAPTERS");
    }, [fileId, navigateToStep, rounds]);
